...
async def list_tasks(user=Depends(get_current_user)):
    return await TaskRepository.get_all(user)
...
async def update_task(uuid:str, payload: UpdateTaskSchema, user=Depends(get_current_user)):
    task = await TaskRepository.update(uuid, payload, user)
    if not task:
        raise HTTPException(404, "Task not found")
    return task
...
async def delete_task(uuid: str, user=Depends(get_current_user)):
    await TaskRepository.soft_delete(uuid, user)